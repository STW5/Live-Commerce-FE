...
export const paymentApi = {
  readyPayment: async (data: PaymentReadyRequest): Promise<PaymentReadyResponse> => {
    const response = await apiClient.post('/api/v2/payments/ready', data);
    return response.data.data;
  },

  approvePayment: async (data: PaymentApproveRequest): Promise<PaymentApproveResponse> => {
    const response = await apiClient.post('/api/v2/payments/approve', data);
    return response.data.data;
  },

  getPayment: async (id: string): Promise<Payment> => {
    const response = await apiClient.get(`/api/v2/payments/${id}`);
    return response.data.data;
  },

  getPayments: async (page: number = 0, size: number = 10): Promise<PageResponse<Payment>> => {
    const response = await apiClient.get('/api/v2/payments', {
      params: { page, size },
    });
    return response.data.data;
  },
  ...
  refundPayment: async (orderId: string): Promise<void> => {
    const response = await apiClient.post(`/api/v2/payments/${orderId}/refund`);
    return response.data.data;
  },

  cancelPayment: async (orderId: string): Promise<void> => {
    await apiClient.post(`/api/v2/payments/${orderId}/cancel`);
  },
};
